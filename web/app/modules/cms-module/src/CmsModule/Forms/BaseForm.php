...
use Devrun\Doctrine\Controls\ToMany;
...
class BaseForm extends Form implements IBaseForm
{
    ...
    /**
     * generate new id from replicator container
     *
     * @param IComponent|Container $container
     * @return int
     */
    public static function generateNewId(IComponent $container)
    {
        $keys = array_keys($container->getValues(true));
        $nonIdKeys = array_filter($keys, function ($key) {
            return Strings::startsWith($key, ToManyContainer::NEW_PREFIX);
        });
        $nonIdAssocKeys = [];
        foreach ($nonIdKeys as $nonIdKey) {
            preg_match("%_new_(\d+)%", $nonIdKey, $matches);
            $nonIdAssocKeys[$nonIdKey] = $matches[1];
        }

        $maxNewId = $nonIdAssocKeys ? max(array_values($nonIdAssocKeys)) : -1;
        $maxNewId++;
        return intval($maxNewId);
    }
